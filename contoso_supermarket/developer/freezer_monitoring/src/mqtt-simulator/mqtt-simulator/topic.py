...
class TopicAuto(Topic, threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            self.client.publish(
                topic=self.topic_url, payload=json.dumps(payload), qos=2, retain=False
            )
    ...
    def generate_data(self):
        ...
        if self.old_payload == None:
            ...
            for data in self.topic_data:
                if data["TYPE"] == "int":
                    payload[data["NAME"]] = random.randint(
                        data["MIN_VALUE"], data["MAX_VALUE"]
                    )
                elif data["TYPE"] == "float":
                    payload[data["NAME"]] = random.uniform(
                        data["MIN_VALUE"], data["MAX_VALUE"]
                    )
                elif data["TYPE"] == "bool":
            ...
            for data in self.topic_data:
                ...
                if data["TYPE"] == "bool":
                    ...
                    payload[data["NAME"]] = (
                        max(payload[data["NAME"]] + step, data["MIN_VALUE"])
                        if step < 0
                        else min(payload[data["NAME"]] + step, data["MAX_VALUE"])
                    )
